...
<head>
    ...
    <title>{{ __('messages.auth.reset_password.title') }} — {{ config('app.name') }}</title>
    ...
    <div class="card shadow-lg border-0" style="width:100%;max-width:420px;">
        <div class="card-body p-4 p-md-5">
            <div class="text-center mb-4">
                <h4 class="mt-3 mb-0">{{ __('messages.auth.reset_password.title') }}</h4>
                <p class="text-muted small mb-0">{{ __('messages.auth.reset_password.subtitle') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.update') }}">
                ...
                <div class="mb-3">
                    <label class="form-label">{{ __('messages.auth.reset_password.confirm') }}</label>
                    <input type="password" name="password_confirmation" class="form-control" required>
                </div>
                <button type="submit" class="btn btn-primary w-100">{{ __('messages.auth.reset_password.submit') }}</button>
            </form>
        </div>
    </div>
</body>

...
export default function StockAdjustmentForm({ branches = [], products = [] }) {
    const t = useT();
    ...
    return (
        <>
            ...
            <PageHeader title={t('pages.stock_adjustments.title')}
                actions={<Link href={route('admin.stock-adjustments.index')} className="btn btn-light">{t('back')}</Link>} />
            <form onSubmit={submit} className="card">
                <div className="card-body">
                    <div className="row">
                        ...
                        <div className="col-md-4">
                            <SelectField label={t('fields.type')} value={data.type} onChange={v => setData('type', v)}
                                options={[{ value: 'addition', label: t('statuses.addition') }, { value: 'subtraction', label: t('statuses.subtraction') }]}
                                error={errors.type} required />
                        </div>
                        <div className="col-12">
                            <TextAreaField label={t('fields.reason')} value={data.reason} onChange={v => setData('reason', v)} error={errors.reason} />
                        </div>
                    </div>
                    <LineItemsEditor items={items} setItems={setItems} products={products} priceField={null} showTax={false} errors={errors} />
                </div>
                <div className="card-footer text-end">
                    <Link href={route('admin.stock-adjustments.index')} className="btn btn-light me-2">{t('cancel')}</Link>
